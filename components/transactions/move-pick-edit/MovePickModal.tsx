...
import { MovePickActions } from "./MovePickActions";
import { MovePickFields } from "./MovePickFields";

type MovePickModalProps = {
  isOpen: boolean;
  onClose: () => void;
};

const MovePickModal = ({ isOpen, onClose }: MovePickModalProps) => {
  ...
  const {
    ...
    handleSubmit,
    selectedProject,
  } = useMovePickModal({ isOpen, onClose });

  const onOpenChange = (open: boolean) => {
    if (!open) handleClose();
  };

  const title = "Move Pick";
  const description = `Move a pick to sit immediately before or after another pick in ${selectedProject?.projectName ?? "this project"}. Changes apply immediately with no preview.`;
