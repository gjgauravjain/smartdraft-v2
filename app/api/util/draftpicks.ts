...
const transformDraft = (data: any): DraftBoardType[] => {
  ...
  const transformDashboardData = data
    ...
    .sort((a: DraftBoardType, b: DraftBoardType) => parseInt(a.rank) - parseInt(b.rank));
  return transformDashboardData;
};

const transformTransactionDescription = (data: any): TransactionDescriptionType => {
  return {
...
const transformMultiPlayersType = (data: any): TransactionMultiPlayersType[] => {
  if (!data) {
    return [];
  }
...
const transformTransactionSource = (transactionType: string, data: any): TransactionSourceType => {
...
const transformTransactionList = (data: any): TransactionListType[] => {
  ...
  return data.map((item: any, index: number) => ({
    ...
    sourceApi: transformTransactionSource(item.Transaction_Type, item.source_api),
  }));
};
...
export const transformDataOrderNewEntry = (data: any): DataOrderEntryType[] => {
  ...
  return data.map(({ TeamNames, Overall_Pick }: any) => {
    const orders = Array.from({ length: Overall_Pick.length }, (_, i) => Overall_Pick[i] || '');
...
export const transformDraftYearList = (data: any): DraftYearList[] => {
  ...
  return data.map((item: Record<string, any>, index: number) => ({
    ...
    currentOwnerShort: item?.current_owner_short || item.Current_Owner_Short_Name || '',
...
export const transformDashboardDraftCurrentYear = (
  ...
  return {
    rd1List: transformDraftYearList(filterData?.filter((item: any) => item.Draft_Round === 'RD1')),
    rd2List: transformDraftYearList(filterData?.filter((item: any) => item.Draft_Round === 'RD2')),
    rd3List: transformDraftYearList(filterData?.filter((item: any) => item.Draft_Round === 'RD3')),
    rd4List: transformDraftYearList(filterData?.filter((item: any) => item.Draft_Round === 'RD4')),
    rd5List: transformDraftYearList(filterData?.filter((item: any) => item.Draft_Round === 'RD5')),
    rd6List: transformDraftYearList(
      filterData?.filter(
        (item: any) => !['RD1', 'RD2', 'RD3', 'RD4', 'RD5'].includes(item.Draft_Round),
      ),
    ),
  };
};
export const transformDashboardFullOrderList = (data: any): DataFullOrderListType[] => {
  if (!data) {
    return [];
  }
