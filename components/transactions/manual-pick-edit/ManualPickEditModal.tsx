...
import { ManualPickEditActions } from "./ManualPickEditActions";
import { ManualPickEditFields } from "./ManualPickEditFields";

type ManualPickEditModalProps = {
  isOpen: boolean;
  onClose: () => void;
};

const ManualPickEditModal = ({ isOpen, onClose }: ManualPickEditModalProps) => {
  ...
  const title = "Manual Pick Edit";
  const description = `Reassign a pick's owner in ${selectedProject?.projectName ?? "this project"}. Changes apply immediately with no preview.`;

  const fields = (
    <ManualPickEditFields
      form={form}
      pickOptions={pickOptions}
      teamOptions={teamOptions}
      picksLoading={picksLoading}
      teamsLoading={teamsLoading}
      sameOwnerError={sameOwnerError}
    />
  );
...
export default ManualPickEditModal;
